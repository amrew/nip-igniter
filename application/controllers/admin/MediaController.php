 ...
 */
class MediaController extends Nip_Controller 
{
		...
		if ($this->input->is_ajax_request()) {
			...
			if(isset($_GET['download'])){
				...
				if($_GET['download'] == "excel"){
					...
					$this->spreadsheet->send("Media spreadsheet.xls");
					...
					$this->dompdflib->generate($view, "Media report.pdf");	
				}
			}else{
	...
	public function edit($id = NULL) {
		...
		if (isset($_POST["Media"])) {
			...
			if($model->method == "url"){
				$model->url = $this->input->post("absolute_url");
			...
			if ($model->validate()) {
				if (!empty($_FILES['url']['name'])) {
					...
					$folder = "./public/uploads/{$this->type}/";
					...
					if (!$this->upload->do_upload('url')) {
						...
						$model->url 	= $folder.$data['file_name'];

						$isCreateThumb  = TRUE;
						$isCrop 		= TRUE;
						
						$arrayImageExt = array('.jpg','.png','.gif');
						if(!in_array($data['file_ext'], $arrayImageExt)){
							$isCreateThumb = FALSE;
							$isCrop 	   = FALSE;
						}
						...
						if($isCrop){
							...
							$this->msg['success']['crop'] = $this->crop($model->url, $callback, $isCreateThumb, $scaleWidth, $scaleHeight);
						}
					}
				}
		...
		$data['allTerm']	= $this->Term->all(array("where"=>"type = '{$this->type}'"));
		$data['allStatus']	= $this->Status->all();
