 ...
 */
class CommentController extends Nip_Controller 
{
    ...
    public function __construct() {
		...
		$this->pageTitle = ucfirst($this->type).' '.$this->pageTitle;
	}
		...
		if ($this->input->is_ajax_request()) {
			...
			if(isset($_GET['download'])){
				...
				if($_GET['download'] == "excel"){
					...
					$field = array('id','name','status_id');

					$this->spreadsheet->send("Comment spreadsheet.xls");
					...
					$this->dompdflib->generate($view, "Comment report.pdf");	
				}
			}else{
	...
	public function edit($id = NULL) {
		...
		if($this->type != 'content'){
			$allPost = $this->Post->all(array('where'=>"type = '{$this->type}'"));
		}else{
			$allPost = $this->Post->all(array('where'=>"type = 'post' OR type = 'page'"));
		}
