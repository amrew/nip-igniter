...
class PostController extends Nip_Controller 
{
    ...
    public function __construct() {
		...
		$this->pageTitle = ucfirst($this->type);
	}
	...
	public function index($limit = NULL, $offset = 0) {
		...
		if (isset($_GET['search'])) {
			if (isset($_GET['keywords']) && isset($_GET['keyword'])) {
				...
				$where    .= " AND " . $this->getSpecificWhere($keywords);
		...
		$data['allTerm']	= $this->Term->all(array('where'=>"type = 'category'"));
		$data['allStatus']	= $this->Status->all();
		$data['allUser']	= $this->User->all();
		$data['allPost']	= $this->Post->all();
		...
		if ($this->input->is_ajax_request()) {
			if(viewExists("{$this->pathController}/page/{$this->type}")){
				$view = $this->renderPartial("{$this->pathController}/page/{$this->type}", $data, TRUE);
			}else{
			...
			if(isset($_GET['download'])){
						...
						foreach($object as $key => $value){
							if(in_array($key, $field)){
								if($key == "category_id"){
									...
									$this->spreadsheet->write($object->post->title);
								}else{
				...
				if(viewExists("{$this->pathController}/index/{$this->type}")){
					$this->render("{$this->pathController}/index/{$this->type}", $data);
				}else{
					$this->render("{$this->pathController}/index", $data);
	...
	public function edit($id = NULL) {
		...
		if(isset($_POST['change_status'])){
			$model->status_id = $this->input->post("status_id");
			if ($model->save()) {
				$this->msg['success']['changeTo'] = $model->status_id;
				$this->msg['success']['title'] = $model->status->title;
				if($model->status_id == 2){
					$this->msg['success']['param'] = 'warning';
				}
		...
		if (isset($_POST["Post"])) {
			...
			$model->author_id = $this->Auth->primary();
			...
			if ($model->validate()) {
					...
					if (!$this->upload->do_upload('image')) {
						...
						if($isCreateThumb){
							...
							//if without cropping
							if(!$isCrop){
								$this->createThumb($imgPath,600,400);
							}
						}
						//if with cropping
						if($isCrop){
				...
				if ($model->save()) {
					$model->saveTags($this->input->post('tags'));
					echo json_encode($this->msg['success']);
				} else {
					echo json_encode($this->msg['failed']);
				}
		...
		$data['allTerm']	= $this->Term->all(array('where'=>"type = 'category'"));
		$data['allTag']		= $this->Term->all(array('where'=>"type = 'tag'"));
		$data['allStatus']	= $this->Status->all();
		$data['allPost']	= $this->Post->all(array('where'=>"type = 'page'"));
		
		$data["id"]			= $id;
		$data["model"]		= $model;
		$data["callback"]	= !empty($_SERVER['HTTP_REFERER'])
		   					 ? $_SERVER['HTTP_REFERER'] : site_url($this->pathController."?type={$this->type}");

		if(viewExists("{$this->pathController}/edit/{$this->type}")){
			$this->render("{$this->pathController}/edit/{$this->type}", $data);
		}else{
	...
	public function delete() {
		...
		if ($result) {
			...
			if($this->Model->getSoftDeletes()){
				$this->msg['success']['message']   = 'Data has been successfully removed. <button class="btn-action btn btn-warning btn-xs" data-id="'.$id.'" data-url="'.site_url("{$this->pathController}/restore?type={$this->type}").'">Undo</button> if this action is a mistake.';
			}else{
