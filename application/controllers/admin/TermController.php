 ...
 */
class TermController extends Nip_Controller 
{
		...
		if ($this->input->is_ajax_request()) {
			...
			if(isset($_GET['download'])){
				...
				if($_GET['download'] == "excel"){
					...
					$this->spreadsheet->send("Term spreadsheet.xls");
					...
					$this->dompdflib->generate($view, "Term report.pdf");	
				}
			}else{
	...
	public function edit($id = NULL) {
		...
		if (isset($_POST["Term"])) {
			...
			if(isset($_POST['quick'])){
				$exist = $this->Model->first(array("title"=>$fields['title']));
				if($exist){
					$model = $exist;
				}
			}
			...
			if ($model->validate()) {	
				if ($model->save()) {
					if(isset($_POST['quick'])){
						$this->msg['success']['data'] = $model;
						$this->msg['success']['id'] = $model->id;
						$this->msg['success']['tags'] = $this->Model->getOptionTags();
		...
		if($this->input->is_ajax_request()){
			$this->renderPartial("{$this->pathController}/edit", $data);
		}else{
