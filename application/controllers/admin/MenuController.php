 ...
 */
class MenuController extends Nip_Controller 
{
	...
	protected function getDefaultSorting(){
		$sorting = "parent_menu_id asc, order asc";
		...
		if ($this->input->is_ajax_request()) {
			...
			if(isset($_GET['download'])){
				...
				if($_GET['download'] == "excel"){
					...
					$field = array('id','title','url','parent_menu_id');

					$this->spreadsheet->send("Menu spreadsheet.xls");
						...
						foreach($object as $key => $value){
							if(in_array($key, $field)){
								if($key == "parent_menu_id"){
									$this->spreadsheet->write($object->menu->title);
								}else{
					...
					$this->dompdflib->generate($view, "Menu report.pdf");	
				}
			}else{
	...
	public function edit($id = NULL) {
		...
		if (isset($_POST["Menu"])) {
			...
			$fields = $_POST["Menu"];
			$model->attr($fields);
			$model->core = isset($fields['core'])?"1":"0";
